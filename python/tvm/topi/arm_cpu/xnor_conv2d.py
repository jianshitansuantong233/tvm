...
from ..nn.xnor_conv2d import xnor_conv2d_legalize
...
@autotvm.register_topi_compute("xnor_conv2d_nhwc.arm_cpu")
...
@autotvm.register_topi_schedule("xnor_conv2d_nhwc.arm_cpu")
def schedule_xnor_conv2d_nhwc(cfg, outs):
    """Arm cpu schedule for xnor conv2d"""
    s = te.create_schedule([x.op for x in outs])

    def _callback(op):
        if "spatial_xnor_conv_nhwc" in op.tag:
            output = op.output(0)
...
@xnor_conv2d_legalize.register("arm_cpu")
def _xnor_conv2d_legalize(attrs, inputs, arg_types):
    """Legalizes xnor Conv2D op.
    ...
    if attrs["data_layout"] == "NHWC":
        ...
        if len(kernel.data.shape) == 4:
            ...
            conv = relay.nn.xnor_conv2d(data, kernel, **new_attrs)
            return conv
    return None
