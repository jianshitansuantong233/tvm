...
@autotvm.register_topi_compute("xnor_conv2d_nchw.x86")
def xnor_conv2d_nchw(
    cfg,
    data,
    kernel,
    stride,
    ...
    return te.compute(
        ...
        name="conv_vec",
        tag="spatial_xnor_conv_nchw",
    )


@autotvm.register_topi_compute("xnor_conv2d_nhwc.x86")
def xnor_conv2d_nhwc(
    cfg,
    data,
    kernel,
    stride,
    ...
    return te.compute(
        ...
        name="output_unpack",
        tag="spatial_xnor_conv_nhwc",
    )


@autotvm.register_topi_schedule("xnor_conv2d_nchw.x86")
def schedule_xnor_conv2d_nchw(cfg, outs):
    return _schedule_xnor_conv2d(cfg, outs)


@autotvm.register_topi_schedule("xnor_conv2d_nhwc.x86")
def schedule_xnor_conv2d_nhwc(cfg, outs):
    return _schedule_xnor_conv2d(cfg, outs)


def _schedule_xnor_conv2d(cfg, outs):
    """CPU schedule for xnor convolutions NCHW and NHWC"""
    ...
    def traverse(op):
        ...
        if tag.is_broadcast(op.tag) or "elemwise" in op.tag:
        ...
        elif "spatial_xnor_conv_nchw" in op.tag or "spatial_xnor_conv_nhwc" in op.tag:
            ...
            if "spatial_xnor_conv_nchw" in op.tag:
                _schedule_xnor_conv2d_nchw(
                    cfg,
                    s,
                    data_q,
                    data_pad,
            ...
            elif "spatial_xnor_conv_nhwc" in op.tag:
                _schedule_xnor_conv2d_nhwc(
                    cfg,
                    s,
                    data_q,
                    data_pad,
...
def _schedule_xnor_conv2d_nchw(
...
def _schedule_xnor_conv2d_nhwc(
    cfg, s, data_q, data_pad, data_vec, kernel_q, kernel_vec, conv_out, output, last
):
